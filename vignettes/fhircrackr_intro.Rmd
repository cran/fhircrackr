...
Of course the previous example is using just one resource type. If you are interested in several types of resources, you use a `fhir_design` containing several `fhir_table_descriptions`.
...
Now our `design` needs two `table_descriptions` (called `MedicationStatements` and `Patients` in our example), one for the MedicationStatement resources and one for the Patient resources:
...
```
In this example, we have spelled out the table description `MedicationStatement` completely, while we have used a short form for `Patients`. The resulting design looks like this:
```{r}
design
```
...
As you can see, the result now contains two tables, one for Patient resources and one for MedicationStatement resources.
