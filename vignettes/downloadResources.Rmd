...
If for some reason your bundles are stored as a character vector containing xml strings (e.g. because you extracted them from somewhere other than a FHIR API), you can use the function `as_fhir()` to coerce this character vector to a `fhir_bundle_list`:
